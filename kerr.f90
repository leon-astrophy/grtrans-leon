...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
#ifdef EXTRA
real(kind=8) :: bl1, bl2, bl3
real(kind=8) :: kl1, kl2, kl3
real(kind=8) :: ahl1, ahl2, ahl3
real(kind=8) :: bhl1, bhl2, bhl3
#endif
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
...
! Eddington luminosity
        ...
        pi=acos(-1.)
        rms=calc_rms(a) 
        y=sqrt(r) ! (r/M)**1/2, Abramowicz P.32
        yms=sqrt(rms) ! (rms/M)**1/2, Abramowicz P.32
        y1=2d0*cos(1./3.*(acos(a)-pi)) ! Abramowicz P.32
        y2=2d0*cos(1./3.*(acos(a)+pi)) ! Abramowicz P.32
        y3=-2d0*cos(1./3.*acos(a)) ! Abramowicz P.32
        arg1=3.*a/(2d0*y) ! Abramowicz P.32
        arg2=3.*(y1-a)**2d0/(y*y1*(y1-y2)*(y1-y3)) ! Abramowicz P.32
        arg3=3.*(y2-a)**2d0/(y*y2*(y2-y1)*(y2-y3)) ! Abramowicz P.32
        arg4=3.*(y3-a)**2d0/(y*y3*(y3-y1)*(y3-y2)) ! Abramowicz P.32
        kc=1.-yms/y-arg1*log(y/yms)-arg2*log((y-y1)/(yms-y1))- &
          arg3*log((y-y2)/(yms-y2))-arg4*log((y-y3)/(yms-y3)) ! Abramowicz P.32
          ! But seems to be Q/Q0/y ... 
...
!  !      write(6,*) 'perpk k'
        gam1=a*cth*k0-a*a*cth*sth*sth*k3 ! Eq. 19 in GRTRANS 2016 paper
        gam2=r*(r*r+a*a)*sth*k3-a*r*sth*k0 ! Eq. 20 in GRTRANS 2016 paper
        gam3=a*a*cth*sth*sth*k1-r*(r*r+a*a)*sth*k2 ! Eq. 21 in GRTRANS 2016 paper
        del1=r*k0-r*a*sth*sth*k3 ! Eq. 16 in GRTRANS 2016 paper
!        del2=a*cth*sth*(r*r+a*a)*k3-a*a*sth*cth*k0
! sign changed 8/24/2015 see notes! leads to small changes ~% usually
        del2=-a*cth*sth*(r*r+a*a)*k3+a*a*sth*cth*k0 ! Eq. 17 in GRTRANS 2016 paper
        del3=r*a*sth*sth*k1+a*cth*sth*(r*r+a*a)*k2 ! Eq. 18 in GRTRANS 2016 paper
...
! changed back to version in mathematica notebook now that i am using correct initial Kap1, Kap2 and not swapped
        denom=(gam2*del1-gam1*del2)*(g33*k3+g03*k0)+(gam3*del2-gam2*del3)*g11*k1-(gam3*del1-gam1*del3)*g22*k2 ! Eq. 28 in GRTRANS 2016 paper
        f1=(gam2*Kap1-del2*Kap2)*(g33*k3+g03*k0)-g22*k2*(gam3*Kap1-del3*Kap2) ! Eq. 22 in GRTRANS 2016 paper
        f2=(del1*Kap2-gam1*Kap1)*(g33*k3+g03*k0)+g11*k1*(gam3*Kap1-del3*Kap2) ! Eq. 24 in GRTRANS 2016 paper
        f3=g22*k2*(gam1*Kap1-del1*Kap2)-g11*k1*(gam2*Kap1-del2*Kap2) ! Eq. 26 in GRTRANS 2016 paper
...
!        f3=g22*k2*(gam1*Kap2-del1*Kap1)-g11*k1*(gam2*Kap2-del2*Kap1)
        ...
        !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
        !
        ! Leon's version on "turning off" general relativistic effect 
        !
        !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
        subroutine transport_perpk_leon(kvec,r,th,a,metric,Kap1,Kap2,f1, &
         f2,f3)
        ! Calculate components of parallel transported vector with
        ! Walker-Penrose constants Kap1, Kap2 at locations 
        ! r,th in Boyer-Lindquist coordinates assuming f0=0.
...
!  !      write(6,*) 'perpk', size(metric,1), size(metric,2)
        ...
        !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
        ! Leon's patch, take the a = 0 limit for the metric !
        g03=0.0d0; g11=r*r/(r*r - 2.0d0*r)
        g22=r*r; g33=r*r*SIN(th)*SIN(th)
        !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
...
!  !      write(6,*) 'perpk k'
        ...
        !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
        ! Leon's patch, take the newtonian limit for the constants !
        gam1=0.0D0 ! Eq. 19 in GRTRANS 2016 paper
        gam2=r*r*r*sth*k3 ! Eq. 20 in GRTRANS 2016 paper
        gam3=-r*r*r*sth*k2 ! Eq. 21 in GRTRANS 2016 paper
        del1=r*k0 ! Eq. 16 in GRTRANS 2016 paper        
        !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
...
! sign changed 8/24/2015 see notes! leads to small changes ~% usually
        ...
        !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
        ! Leon's patch, take the newtonian limit for the constants !
        del2=0.0d0 ! Eq. 17 in GRTRANS 2016 paper
        del3=0.0d0 ! Eq. 18 in GRTRANS 2016 paper
        !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
...
!        f3=g22*k2*(gam1*Kap2-del1*Kap1)-g11*k1*(gam2*Kap2-del2*Kap1)
        where(abs(denom).gt.0d0)
           ...
           f3=f3/denom
        endwhere
        end subroutine transport_perpk_leon

#endif

#ifdef LEON
        subroutine comoving_ortho(r,th,a,alpha,beta,mus,u,b,k, &
                s2xi,c2xi,ang,g,cosne, cosrho0, gam_relv)
#else
        subroutine comoving_ortho(r,th,a,alpha,beta,mus,u,b,k, &
        ...
        real(kind=8), dimension(size(r)) :: tmpry
        real(kind=8), dimension(size(r),3) :: aahat
#ifdef LEON
        ! Leon's patch !
        real(kind=8), intent(out), dimension(size(r)) :: cosrho0, gam_relv
        CALL zamo_ortho(r,th,a,alpha,beta,mus,u,b,k,s2xi,c2xi,g,cosrho0,gam_relv)
#else
        ...
        !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
        !
        ! Leon's patch on projecting vectors to ZAMO frame
        !
        !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
        subroutine zamo_ortho(r,th,a,alpha,beta,mus,u,b,k,s2xi,c2xi,g,cosrho0,gam_relv)
        USE compton, only: grmhd, INTERPOLATE_VEL, check_transport, aspin_leon, rbh_leon

        !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!

        ! 4-vectors for velocity, magnetic fields, and photon !
        type (four_Vector), dimension(:), intent(in) :: k
        type (four_Vector), dimension(:) :: b, u
        
        ! r and theta in BL coordinates !
        real(kind=8), dimension(:), intent(in) :: r, th

        ! Impact parameter and spins !
        real(kind=8), intent(in) :: a, alpha, beta, mus

        ! Rotation angles and redshift 
        real(kind=8), intent(out), dimension(size(r)) :: s2xi, c2xi, g, cosrho0, gam_relv

        !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!

        ! Four vector in ZAMO frame !
        type (four_Vector), dimension(size(r)) :: uhat, bhat, khat, aa

        ! Metric and its transpose !
        real(kind=8), dimension(size(r),10) :: metric
        real(kind=8), dimension(10,size(r)) :: tmetric
        ...
        real(kind=8), dimension(size(r)) :: bhatt, bhatr, bhatm, bhatp
        ...
        ! velocity !
        real(kind=8) :: ut_loc, ur_loc, um_loc, up_loc
        ...
        ! Walker-penrose constants, see GRTRANS 2016 !
        ...
        sig_bl = r*r + a*a*COS(th)*COS(th)
        a_bl = (r*r + a*a)*(r*r + a*a) - a*a*delta_bl*SIN(th)*SIN(th)

        !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!

        ! Leon's patch, interpolate 4-velocity coordinate velocity !
        IF(grmhd) THEN
          CALL INTERPOLATE_VEL(r(1), th(1), a, ut_loc, ur_loc, um_loc, up_loc)
          u(:)%data(1) = ut_loc; u(:)%data(2) = ur_loc; u(:)%data(3) = um_loc; u(:)%data(4) = up_loc
        END IF
        ...
        ! JAD 10/5/2015
        ! Kap1, Kap2 changed to be consistent with my math. accompanying change in 
        ! transport_perpk leaves everything unchanged and makes it more clear.
        ...
        ! aa = f^mu in Eq. 3 of GRTRANS 2016 paper
        ...
        ! Normailize !
        ekt%data(1)= 0.0d0; ekt%data(2)= 0.0d0; ekt%data(3)= 0.0d0; ekt%data(4)= 0.0d0
        ekr%data(1)= 0.0d0; ekr%data(2)= 0.0d0; ekr%data(3)= 0.0d0; ekr%data(4)= 0.0d0
        ekm%data(1)= 0.0d0; ekm%data(2)= 0.0d0; ekm%data(3)= 0.0d0; ekm%data(4)= 0.0d0
        ekp%data(1)= 0.0d0; ekp%data(2)= 0.0d0; ekp%data(3)= 0.0d0; ekp%data(4)= 0.0d0

        ! My patch, projecting to the ZAMO frame !
        ! See Bardeen 1972 for the projecting vector !
        ekt%data(1)= SQRT(a_bl/sig_bl/delta_bl)
        ekt%data(4)= 2*a*r/SQRT(a_bl*sig_bl*delta_bl)
        ekr%data(2)= SQRT(delta_bl/sig_bl)
        ekm%data(3)= 1.0d0/SQRT(sig_bl)
        ekp%data(4)= SQRT(sig_bl/a_bl)/SIN(th)
        ...
        ! Transforming to the ZAMO frame !
        ! Remember to raise the index of the time component !
        ! Which is just a simple minus sign in flat space !
        ...
        ! Offset to produce small letter ahat in Shcherbakov Huang 2011, Eq.70 ! 
        ...
        ! perform cross product to get small letter bhat in Shcherbakov Huang 2011, Eq.71 ! 
        ...
        !WRITE (*,*) aa*k
        !WRITE (*,*) - khatt*ahatt + khatr*ahatr + khatm*ahatm + khatp*ahatp
        !WRITE (*,*) aahat(:,1)*khatr + aahat(:,2)*khatm + aahat(:,3)*khatp
        !WRITE (*,*) bbhat(:,1)*khatr + bbhat(:,2)*khatm + bbhat(:,3)*khatp
        ! Assign rotation angle !
        ! Note, if the velocity is zero, no polarization, and thus we assign an arbitary rotation angle to prevent NAN !
        where(vsq.gt.0d0)
...
! Kap1, Kap2 changed to be consistent with my math. accompanying change in transport_perpk leaves everything unchanged and makes it more clear.
        ...
        aa%data(1)=0d0;aa%data(2)=al1;aa%data(3)=al2;aa%data(4)=al3 ! aa = f^mu in Eq. 3 of GRTRANS 2016 paper
        call assign_metric(aa,tmetric)

! Kulkarni+2011 normalizations, basis projection to fluid frame
        delta=r*r+a*a-2.*r
...
! Transforming to the fluid frame !
        uhatt=ekt*u; uhatr=ekr*u; uhatm=ekm*u; uhatp=ekp*u
        bhatt=ekt*b; bhatr=ekr*b; bhatm=ekm*b; bhatp=ekp*b
        khatt=ekt*k; khatr=ekr*k; khatm=ekm*k; khatp=ekp*k
        ahatt=ekt*aa; ahatr=ekr*aa; ahatm=ekm*aa; ahatp=ekp*aa

! Assign 4-velocity in the fluid frame !
        uhat%data(1)=uhatt; uhat%data(2)=uhatr; uhat%data(3)=uhatm; uhat%data(4)=uhatp
...
!        write(6,*) 'vector ortho: ',b(1)%data,u(1)%data,k(1)%data
        bdotk=b*k; bdotb=b*b
        om=-k*u; om2=om*om ! Not sure what happen 
...
! if field is zero, emissivity = 0 so arbitrary angles to prevent NaNs
        ...
        elsewhere
           ...
           angnorm=0.5d0
        endwhere
        ang=acos(merge(merge(angnorm,angmax,angnorm.le.angmax),angmin,angnorm.ge.angmin))
        
        ! CHANGED SIGN following \nu = -u^\mu k_\mu = -u^t k_t = k^t in comoving ortho so g = 1 / k^t
        g=1d0/khat%data(1)
        cosne=g*sqrt(beta*beta+mus*mus*(alpha*alpha-a*a))/r ! note, beta*beta+mus*mus*(alpha*alpha-a*a) is carter's constant
...
! (see 2/22 notes, Chandrasekhar 1983 and CS77, CPS80)
        Kap1=beta
        Kap2=-alpha-a*sqrt(1.-mus*mus) ! See paragraph after Eq. 34 of 2016 grtrans paper
...
! LNRF momentum:
      ...
      ! find momentum vector in tetrad notations !
      ! the inputs are not LNRF momentum but photon 4-vector !
      ptt=r*sqrt(delta/AR)*p%data(1) ! Agol 1997 Eq 5.23
      prt=r/sqrt(delta)*p%data(2) ! Agol 1997 Eq 5.24
      ptht=r*p%data(3) ! Agol 1997 Eq 5.25
      ppht=sqrt(AR)/r*(p%data(4)-om*p%data(1)) ! Agol 1997 Eq 5.26

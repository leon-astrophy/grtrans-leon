...
!       use interpolate, only: get_weight, locate
       ...
       use kerr, only: comoving_ortho, calc_polar_psi, calc_kb_ang, calcg, lnrf_frame, &
            comoving_ortho_debug, bl1, bl2, bl3, kl1, kl2, kl3, ahl1, ahl2, ahl3, bhl1, bhl2, bhl3
#else
       use kerr, only: comoving_ortho, calc_polar_psi, calc_kb_ang, calcg, lnrf_frame, &
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
! Leon's patch. I need relativistic gamma and cosrho0 for ICS
#ifdef LEON
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
! Leon's patch. I override f%b from the get_fluid_vars
#ifdef LEON
            allocate(gam_relv(g%npts))
            allocate(cosrho0(g%npts))
            allocate(r_in(g%npts))
            call comoving_ortho(g%x%data(2),g%x%data(3),g%gk%a, &
                 g%gk%alpha(1),g%gk%beta(1),g%gk%mu0,f%u,f%b,g%k,s2xi,c2xi, &
                 ang,rshift,cosne,cosrho0,gam_relv)
#else
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
! Leon's patch. I calculate invariant emissitivity for the unrotated stokes 
#ifdef LEON
                        i0 = i0*rshift(1)**npow
#endif
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
